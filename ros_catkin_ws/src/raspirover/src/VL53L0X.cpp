...
int wiringPiI2CReadRegBlock(int fd, uint8_t reg, int* buffer, int num)
{
	int i=0;
	while(i<num)
	{
		buffer[i] = wiringPiI2CReadReg16(fd,reg++);
		i++;
	}
	return i;
}

int wiringPiI2CWriteRegBlock(int fd, uint8_t reg, int* buffer, int num)
{
	int i=0;
	while(i<num)
	{
		wiringPiI2CWriteReg16(fd,reg++,buffer[i]);
		i++;
	}
	return i;
}
